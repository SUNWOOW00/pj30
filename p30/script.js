...
function growCircles(i, j) {
    if (i >= 0 && i < cols && j >= 0 && j < rows && circlesArr[i][j] && !circlesArr[i][j].classList.contains('grow')) {
...
function makeRandomWave() {
    const randomCoords = [Math.floor(Math.random() * cols), Math.floor(Math.random() * rows)]; // 이 줄을 수정했습니다.
